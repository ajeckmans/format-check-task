...
async function runFormatCheck(settings: Settings): Promise<boolean> {
    ...
    if (settings.Parameters.scopeToPullRequest) {
        ...
        annotatedReports = annotatedReports.map(report => {
            const change = changedInPR.find(c => c.FilePath === report.FilePath);
            if (change && change.lineChanges.length > 0) {
                report.FileChanges = report.FileChanges.filter(changeData =>
                    change.lineChanges.includes(changeData.LineNumber)
                );
            }
            return report;
        }).filter(report => report.FileChanges.length > 0);
    }
...
async function getChangedFilesInPR(pullRequestUtils: PullRequestService, settings: Settings): Promise<PullRequestFileChanges> {
    ...
    for (const change of pullRequestChanges as ExtendedGitChange[]) {
        ...
        files.push(new PullRequestFileChange(normalizedPath, change.item?.commitId!, change.changeType!, lineChanges));
    }

    console.log("All changed files considered to be part of this Pull Request: ");
    files.forEach(file => {
        console.log(`${file.FilePath} - ${file.changeType} - ${file.CommitId}`);
        if (file.lineChanges.length > 0) {
            console.log(`  Changed lines: ${file.lineChanges.join(', ')}`);
        }
    });
    return files;
}
